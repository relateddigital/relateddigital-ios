...
final class Form {
    ...
    static var defaultNavigationOptions = RowNavigationOptions.Enabled.union(.SkipCanNotBecomeFirstResponderRow)

    /// The default options that define when an inline row will be hidden. Applies only when `inlineRowHideOptions` is nil.
    static var defaultInlineRowHideOptions = InlineRowHideOptions.FirstResponderChanges.union(.AnotherInlineRowIsShown)

    /// The options that define when an inline row will be hidden. If nil then `defaultInlineRowHideOptions` are used
    var inlineRowHideOptions: InlineRowHideOptions?

    /// Which `UIReturnKeyType` should be used by default. Applies only when `keyboardReturnType` is nil.
    static var defaultKeyboardReturnType = KeyboardReturnTypeConfiguration()
     ...
     */
    subscript(indexPath: IndexPath) -> BaseRow {
     ...
     */
    func rowBy<T>(tag: String) -> RowOf<T>? where T: Equatable{
    ...
    /**
     Returns the row whose tag is passed as parameter. Uses a dictionary to get the row faster
     */
    func rowBy<Row>(tag: String) -> Row? where Row: RowType{
        let row: BaseRow? = rowBy(tag: tag)
        return row as? Row
    }
     ...
     */
    func sectionBy(tag: String) -> Section? {
     ...
     */
    func values(includeHidden: Bool = false) -> [String: Any?] {
        if includeHidden {
    ...
    var rows: [BaseRow] { return flatMap { $0 } }

    /// All the rows of this form. Includes the hidden rows.
    var allRows: [BaseRow] { return kvoWrapper._allSections.map({ $0.kvoWrapper._allRows }).flatMap { $0 } }

    /// All the sections of this form. Includes hidden sections.
    var allSections: [Section] { return kvoWrapper._allSections }
...
extension Form: Collection {
    ...
    var endIndex: Int { return kvoWrapper.sections.count }
}
...
extension Form: MutableCollection {
    ...
    subscript (_ position: Int) -> Section {
...
extension Form : RangeReplaceableCollection {
    ...
    func append(_ formSection: Section) {
    ...
    func append<S: Sequence>(contentsOf newElements: S) where S.Iterator.Element == Section {
    ...
    func replaceSubrangeInAllSections<C: Collection>(
        _ subRange: Range<Int>,
        with newElements: C
    ) where C.Iterator.Element == Section {
    ...
    func removeAll(where shouldBeRemoved: (Section) throws -> Bool) rethrows {
...
extension Form {
    ...
    @discardableResult
    func validate(includeHidden: Bool = false, includeDisabled: Bool = true, quietly: Bool = false) -> [ValidationError] {
