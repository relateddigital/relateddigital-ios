...
extension Section {
     ...
     */
    func rowBy<Row: RowType>(tag: String) -> Row? {
...
class Section {
    ...
    var header: HeaderFooterViewRepresentable? {
        willSet {
            headerView = nil
        }
    }
    ...
    var footer: HeaderFooterViewRepresentable? {
        willSet {
            footerView = nil
        }
    }
    ...
    var index: Int? { return form?.firstIndex(of: self) }
    ...
    init(_ initializer: @escaping (Section) -> Void) {
        initializer(self)
    }

    init(_ header: String?, _ initializer: @escaping (Section) -> Void = { _ in }) {
    ...
    init(header: String?, footer: String?, _ initializer: (Section) -> Void = { _ in }) {
        if let header = header {
            self.header = HeaderFooterView(stringLiteral: header)
        }
    ...
    init(footer: String?, _ initializer: (Section) -> Void = { _ in }) {
        if let footer = footer {
            self.footer = HeaderFooterView(stringLiteral: footer)
        }
        initializer(self)
...
extension Section: MutableCollection, BidirectionalCollection {
    ...
    var endIndex: Int { return kvoWrapper.rows.count }
    subscript (position: Int) -> BaseRow {
        get {
    ...
    subscript (range: Range<Int>) -> ArraySlice<BaseRow> {
...
extension Section: RangeReplaceableCollection {
    ...
    func replaceSubrange<C>(_ subrange: Range<Int>, with newElements: C) where C : Collection, C.Element == BaseRow {
    ...
    func removeAll(where shouldBeRemoved: (BaseRow) throws -> Bool) rethrows {
...
extension Section /* Helpers */ {
     ...
     */
    func insert(row newRow: BaseRow, after previousRow: BaseRow) throws {
 ...
 */
struct MultivaluedOptions: OptionSet {
    ...
    /// No multivalued.
    static let None = MultivaluedOptions(.none)

    /// Allows user to insert rows.
    static let Insert = MultivaluedOptions(.insert)

    /// Allows user to delete rows.
    static let Delete = MultivaluedOptions(.delete)

    /// Allows user to reorder rows
    static let Reorder = MultivaluedOptions(.reorder)
}

/// Base class for multivalued sections. Use one of the subclasses.
class BaseMultivaluedSection: Section {
    var multivaluedOptions: MultivaluedOptions
    var showInsertIconInAddButton = true

    var multivaluedRowToInsertAt: ((Int) -> BaseRow)?

    required init(multivaluedOptions: MultivaluedOptions = MultivaluedOptions.Insert.union(.Delete),
                         header: String? = nil,
                         footer: String? = nil,
...
class GenericMultivaluedSection<AddButtonType: RowType>: BaseMultivaluedSection where AddButtonType: BaseRow {

    var addButtonProvider: ((GenericMultivaluedSection<AddButtonType>) -> AddButtonType)!
 ...
 */
class MultivaluedSection: GenericMultivaluedSection<ButtonRow> {
