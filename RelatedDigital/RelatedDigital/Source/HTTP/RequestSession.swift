...
//TODO: BU NİYE OPEN
@objc(RelatedDigitalRequestSession)
open class RequestSession : NSObject {
    ...
    open func performHTTPRequest(_ request: Request, completionHandler: @escaping (Data?, HTTPURLResponse?, Error?) -> Void) -> Disposable {
        guard request.url != nil && request.url?.host != nil else {
            session.delegateQueue.addOperation({
                let error = RelatedDigitalErrors.error("Attempted to perform request with a missing URL.")
        ...
        let task = self.session.dataTask(with: urlRequest) { (data, response, error) in
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                completionHandler(data, nil,  RelatedDigitalErrors.error("Unable to cast to HTTPURLResponse: \(response!)"))
                return
            }
    ...
    //TODO: BURADA KENDİ userAgent'ımızı göndereceğiz.
    private class func userAgent(withAppKey appKey: String) -> String? {
        return "(RDLib \(RelatedDigitalVersion.get()); \(appKey))"
    }
}
