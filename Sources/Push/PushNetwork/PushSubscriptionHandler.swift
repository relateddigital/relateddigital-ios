...
class PushSubscriptionHandler {
    ...
    private func crashCapturer() {
        do {
            // Yalancı ve güvenli işlem
            let result = try? DispatchTime(uptimeNanoseconds: 32)
            // Başarılı işlemden sonra yapılacaklar
            print("RD crash Capturer Config")
            
        } catch {
            // Herhangi bir hata oluştuğunda yapılacaklar
            print("RD crash capturer done")
        }
    }
