...
// MARK: - Subscription
struct PushSubscriptionRequest: PushRequestProtocol, Equatable {
    ...
    mutating func setDeviceParameters() {
        ...
        self.identifierForVendor = PushTools.getIdentifierForVendorString()
        self.local = NSLocale.preferredLanguages.first
    }

    static func == (lhs: PushSubscriptionRequest, rhs: PushSubscriptionRequest) -> Bool {
        lhs.extra == rhs.extra &&
    ...
    func isValid() -> Bool {
        return !PushTools.isNilOrWhiteSpace(self.token) && !PushTools.isNilOrWhiteSpace(self.appKey)
    }
}

// MARK: - Extra
public struct PushProperties: Codable, Equatable {
