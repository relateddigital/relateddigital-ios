//
//  RDCarouselFullscreenViewController.swift
//  RelatedDigitalIOS
//

import UIKit

private enum RDCarouselFullscreenStyle {
...
private final class RDCarouselFullscreenCell: UICollectionViewCell {
    ...
    func configure(item: RDCarouselItem) {
        titleLabel.text = item.title?.removeEscapingCharacters()
        titleLabel.font = item.titleFont
        titleLabel.textColor = item.titleColor ?? RDCarouselFullscreenStyle.defaultTitle

        bodyLabel.text = item.body?.removeEscapingCharacters()
        bodyLabel.font = item.bodyFont
        bodyLabel.textColor = item.bodyColor ?? RDCarouselFullscreenStyle.defaultBody
...
final class RDCarouselFullscreenViewController: RDBaseNotificationViewController,
    UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

    private let carouselItems: [RDCarouselItem]
    ...
    var onButtonTap: ((_ notification: RDInAppNotification, _ link: String?, _ button: RDCarouselFullscreenButton, _ index: Int) -> Void)?

    init(notification: RDInAppNotification, carouselItems: [RDCarouselItem]) {
    ...
    override func viewDidLoad() {
        ...
        collectionView.register(RDCarouselFullscreenCell.self, forCellWithReuseIdentifier: "fullscreenCarouselCell")
        ...
        pageControl.currentPageIndicatorTintColor = RDCarouselFullscreenStyle.defaultTitle
        ...
        if let closeImg = RDHelper.getUIImage(named: "VisilabsCloseButton") {
    ...
    private func handleButton(slot: RDCarouselFullscreenButton) {
    ...
    private func updateFooter(for index: Int) {
        ...
        pageControl.currentPageIndicatorTintColor = item.titleColor ?? RDCarouselFullscreenStyle.defaultTitle

        let radius = CGFloat(Float(notification?.buttonBorderRadius ?? "14") ?? 14)
        primaryButton.layer.cornerRadius = radius
        secondaryButton.layer.cornerRadius = radius

        let primaryFill = item.buttonColor ?? RDCarouselFullscreenStyle.defaultPrimaryFill
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "fullscreenCarouselCell", for: indexPath) as! RDCarouselFullscreenCell
