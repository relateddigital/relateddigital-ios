//
//  RDGiftBoxCodeBannerController.swift
//  RelatedDigitalIOS
//
...
class RDGiftBoxCodeBannerController: RDBaseNotificationViewController {
    
    var RDGiftBoxBannerView: RDGiftBoxCodeBannerView!
    ...
    init(_ giftBox: GiftBoxModel) {
        ...
        RDGiftBoxBannerView = RDGiftBoxCodeBannerView(frame: UIScreen.main.bounds, giftBox: self.giftBox!)
        view = RDGiftBoxBannerView
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(didTap(gesture:)))
        tapGesture.numberOfTapsRequired = 1
        RDGiftBoxBannerView.addGestureRecognizer(tapGesture)
        
        let closeTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(closeButtonTapped(tapGestureRecognizer:)))
        RDGiftBoxBannerView.closeButton.isUserInteractionEnabled = true
        RDGiftBoxBannerView.closeButton.addGestureRecognizer(closeTapGestureRecognizer)
    }
    ...
    @objc func didTap(gesture: UITapGestureRecognizer) {
        if !isDismissing && gesture.state == UIGestureRecognizer.State.ended {
            UIPasteboard.general.string = giftBox?.promocode_banner_text.replacingOccurrences(of: "\'", with: "")
    ...
    override func show(animated: Bool) {
        ...
        halfScreenHeight = Double(RDGiftBoxBannerView.horizontalStackView.frame.height)
