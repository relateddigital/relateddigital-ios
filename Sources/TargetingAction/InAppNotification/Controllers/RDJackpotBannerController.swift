//
//  RDJackpotBannerController.swift
//  RelatedDigitalIOS
//
...
class RDJackpotBannerController: RDBaseNotificationViewController {
    
    var jackpotCodeBannerView: RDJackpotCodeBannerView!
    ...
    init(_ jackpotModel: JackpotModel) {
        ...
        jackpotCodeBannerView = RDJackpotCodeBannerView(frame: UIScreen.main.bounds, jackpotModel:  self.jackpot!)
        view = jackpotCodeBannerView
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(didTap(gesture:)))
        tapGesture.numberOfTapsRequired = 1
        jackpotCodeBannerView.addGestureRecognizer(tapGesture)
        
        let closeTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(closeButtonTapped(tapGestureRecognizer:)))
        jackpotCodeBannerView.closeButton.isUserInteractionEnabled = true
        jackpotCodeBannerView.closeButton.addGestureRecognizer(closeTapGestureRecognizer)
    }
    ...
    @objc func didTap(gesture: UITapGestureRecognizer) {
        if !isDismissing && gesture.state == UIGestureRecognizer.State.ended {
            UIPasteboard.general.string = jackpot?.promocode_banner_text.replacingOccurrences(of: "\'", with: "")
    ...
    override func show(animated: Bool) {
        ...
        halfScreenHeight = Double(jackpotCodeBannerView.horizontalStackView.frame.height)
