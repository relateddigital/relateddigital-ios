//
//  RDPlinkoCodeBannerController.swift
//  RelatedDigitalIOS
//
//  Plinko oyunu kapandıktan sonra kazanılan promosyon kodunu üstte banner
//  olarak gösterir. RDClawMachineBannerController baz alınmıştır.
//

import UIKit

class RDPlinkoCodeBannerController: RDBaseNotificationViewController {

    var plinkoCodeBannerView: RDPlinkoCodeBannerView!
    var halfScreenHeight = 0.0
    var isDismissing = false

    init(_ plinkoModel: PlinkoModel) {
        super.init(nibName: nil, bundle: nil)
        self.plinko = plinkoModel
        plinkoCodeBannerView = RDPlinkoCodeBannerView(frame: UIScreen.main.bounds, plinkoModel: self.plinko!)
        view = plinkoCodeBannerView

        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(didTap(gesture:)))
        tapGesture.numberOfTapsRequired = 1
        plinkoCodeBannerView.addGestureRecognizer(tapGesture)

        let closeTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(closeButtonTapped(tapGestureRecognizer:)))
        plinkoCodeBannerView.closeButton.isUserInteractionEnabled = true
        plinkoCodeBannerView.closeButton.addGestureRecognizer(closeTapGestureRecognizer)
    }
    ...
    @objc func didTap(gesture: UITapGestureRecognizer) {
        if !isDismissing && gesture.state == UIGestureRecognizer.State.ended {
            UIPasteboard.general.string = BannerCodeManager.shared.getPlinkoCode()
    ...
    override func show(animated: Bool) {
        ...
        halfScreenHeight = Double(plinkoCodeBannerView.horizontalStackView.frame.height)
