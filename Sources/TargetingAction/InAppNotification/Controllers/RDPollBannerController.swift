...
class RDPollBannerController: RDBaseNotificationViewController {
    
    var pollCodeBannerView: RDPollCodeBannerView!
    ...
    init(_ pollModel: PollModel) {
        ...
        self.poll = pollModel
        pollCodeBannerView = RDPollCodeBannerView(frame: UIScreen.main.bounds, pollModel:  self.poll!)
        view = pollCodeBannerView
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(didTap(gesture:)))
        tapGesture.numberOfTapsRequired = 1
        pollCodeBannerView.addGestureRecognizer(tapGesture)
        
        let closeTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(closeButtonTapped(tapGestureRecognizer:)))
        pollCodeBannerView.closeButton.isUserInteractionEnabled = true
        pollCodeBannerView.closeButton.addGestureRecognizer(closeTapGestureRecognizer)
    }
    ...
    @objc func didTap(gesture: UITapGestureRecognizer) {
        if !isDismissing && gesture.state == UIGestureRecognizer.State.ended {
            UIPasteboard.general.string = poll?.promocode_banner_text.replacingOccurrences(of: "\'", with: "")
    ...
    override func show(animated: Bool) {
        ...
        halfScreenHeight = Double(pollCodeBannerView.horizontalStackView.frame.height)
