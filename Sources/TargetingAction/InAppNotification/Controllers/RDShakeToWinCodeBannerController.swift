//
//  RDShakeToWinCodeBannerController.swift
//  RelatedDigitalIOS
//
...
class RDShakeToWinCodeBannerController: RDBaseNotificationViewController {
    
    var RDShakeToWinBannerView: RDShakeToWinCodeBannerView!
    var halfScreenHeight = 0.0
    
    var isDismissing = false
    
    init(_ shakeToWin: ShakeToWinViewModel) {
        super.init(nibName: nil, bundle: nil)
        self.shakeToWin = shakeToWin
        RDShakeToWinBannerView = RDShakeToWinCodeBannerView(frame: UIScreen.main.bounds, shakeToWin:  self.shakeToWin!)
        view = RDShakeToWinBannerView
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(didTap(gesture:)))
        tapGesture.numberOfTapsRequired = 1
        RDShakeToWinBannerView.addGestureRecognizer(tapGesture)
        
        let closeTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(closeButtonTapped(tapGestureRecognizer:)))
        RDShakeToWinBannerView.closeButton.isUserInteractionEnabled = true
        RDShakeToWinBannerView.closeButton.addGestureRecognizer(closeTapGestureRecognizer)
    }
    ...
    @objc func didTap(gesture: UITapGestureRecognizer) {
        if !isDismissing && gesture.state == UIGestureRecognizer.State.ended {
            UIPasteboard.general.string = shakeToWin?.promocode_banner_text?.replacingOccurrences(of: "\'", with: "")
    ...
    override func show(animated: Bool) {
        ...
        halfScreenHeight = Double(RDShakeToWinBannerView.horizontalStackView.frame.height)
